...
// Функция для установки цвета подсветки клавиш в зависимости от активного слоя
void rgb_matrix_set_key_layer(uint8_t layer);

layer_state_t layer_state_set_user(layer_state_t state) {
    uint8_t active_layer = get_highest_layer(state);

    // Устанавливаем цвет подсветки для слоя
    switch (active_layer) {
        case LAYER_LOWER:
        ...
        case LAYER_POINTER:
            ...
            break;
        default:
            rgb_matrix_reload_from_eeprom();  // Восстанавливаем настройки по умолчанию для других слоев
            break;
    }

    // Настройка подсветки клавиш в зависимости от активного слоя
    rgb_matrix_set_key_layer(active_layer);

    return state;
}

// Функция для установки подсветки клавиш в зависимости от слоя
void rgb_matrix_set_key_layer(uint8_t layer) {
    for (uint8_t row = 0; row < MATRIX_ROWS; ++row) {
        for (uint8_t col = 0; col < MATRIX_COLS; ++col) {
            ...
            if (index != NO_LED) {
                // Устанавливаем цвет подсветки в зависимости от активного слоя
                switch (layer) {
                    case LAYER_LOWER:
